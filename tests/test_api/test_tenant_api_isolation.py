...
class TestEndpointScopeCalls:
    """验证每个 endpoint 的源码中包含正确的 require_scope 调用。"""

    # -- products endpoints --

    def test_create_product_calls_require_scope_write(self) -> None:
        """create_product 应调用 require_scope(auth, "products:write")。"""
        from src.api.router.products import create_product

        source = inspect.getsource(create_product)
        assert (
            'require_scope(auth, "products:write")' in source
            or "require_scope(auth, 'products:write')" in source
        ), f"create_product 未调用 require_scope(auth, 'products:write')\nsource:\n{source}"

    def test_list_products_calls_require_scope_read(self) -> None:
        """list_products 应调用 require_scope(auth, "products:read", "products:write")。"""
        ...
        assert "require_scope" in source, f"list_products 未调用 require_scope\nsource:\n{source}"

    def test_get_product_calls_require_scope_read(self) -> None:
        """get_product 应调用 require_scope(auth, "products:read", "products:write")。"""
    ...
    def test_update_product_calls_require_scope_write(self) -> None:
        """update_product 应调用 require_scope(auth, "products:write")。"""
    ...
    def test_delete_product_calls_require_scope_write(self) -> None:
        """delete_product 应调用 require_scope(auth, "products:write")。"""
    ...
    def test_upload_product_image_calls_require_scope_write(self) -> None:
        """upload_product_image 应调用 require_scope(auth, "products:write")。"""
    ...
    def test_create_task_calls_require_scope_write(self) -> None:
        """create_task 应调用 require_scope(auth, "tasks:write")。"""
        from src.api.router.tasks import create_task

        source = inspect.getsource(create_task)
        assert (
            'require_scope(auth, "tasks:write")' in source
            or "require_scope(auth, 'tasks:write')" in source
        ), f"create_task 未调用 require_scope(auth, 'tasks:write')\nsource:\n{source}"

    def test_list_tasks_calls_require_scope_read(self) -> None:
        """list_tasks 应调用 require_scope(auth, "tasks:read", "tasks:write")。"""
    ...
    def test_get_task_detail_calls_require_scope_read(self) -> None:
        """get_task_detail 应调用 require_scope(auth, "tasks:read", "tasks:write")。"""
    ...
    def test_get_task_status_calls_require_scope_read(self) -> None:
        """get_task_status 应调用 require_scope(auth, "tasks:read", "tasks:write")。"""
    ...
    def test_cancel_task_calls_require_scope_write(self) -> None:
        """cancel_task 应调用 require_scope(auth, "tasks:write")。"""
    ...
    def test_delete_task_calls_require_scope_write(self) -> None:
        """delete_task 应调用 require_scope(auth, "tasks:write")。"""
    ...
    def test_task_websocket_calls_require_scope_read(self) -> None:
        """task_websocket 应调用 require_scope(auth, "tasks:read", "tasks:write")。"""
        from src.api.router.tasks import task_websocket

        source = inspect.getsource(task_websocket)
        assert "tasks:read" in source, f"task_websocket 源码中未找到 tasks:read\nsource:\n{source}"
        assert "tasks:write" in source
        assert "require_scope" in source
...
class TestTaskManagerTenant:
    ...
    def test_create_task_has_tenant_id_param(self) -> None:
        ...
        assert "tenant_id" in params, (
            f"TaskManager.create_task 缺少 tenant_id 参数，当前参数: {params}"
        )
